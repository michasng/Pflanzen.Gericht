...
BEGIN
  ...
  ORDER BY
    ...
    CASE WHEN p_sort = 'fat_asc' THEN (
      ...
      ORDER BY pn.amount_micrograms DESC LIMIT 1
    ) END ASC NULLS LAST,
    CASE WHEN p_sort = 'saturated_fat_asc' THEN (
      SELECT pn.amount_micrograms FROM public.product_nutrient pn
      WHERE pn.product_id = p.id
        AND lower(trim(pn.name)) = ANY (ARRAY['gesättigte fettsäuren', 'davon gesättigte fettsäuren'])
      ORDER BY pn.amount_micrograms DESC LIMIT 1
    ) END ASC NULLS LAST,
    CASE WHEN p_sort = 'sugar_asc' THEN (
      SELECT pn.amount_micrograms FROM public.product_nutrient pn
      WHERE pn.product_id = p.id AND lower(trim(pn.name)) = ANY (ARRAY['zucker', 'davon zucker'])
      ORDER BY pn.amount_micrograms DESC LIMIT 1
    ) END ASC NULLS LAST,
    CASE WHEN p_sort = 'protein_desc' THEN (
      SELECT pn.amount_micrograms FROM public.product_nutrient pn
      WHERE pn.product_id = p.id AND lower(trim(pn.name)) = ANY (ARRAY['eiweiß', 'eiweiss', 'protein'])
      ORDER BY pn.amount_micrograms DESC LIMIT 1
    ) END DESC NULLS LAST,
    CASE WHEN p_sort = 'fiber_desc' THEN (
      SELECT pn.amount_micrograms FROM public.product_nutrient pn
      WHERE pn.product_id = p.id AND lower(trim(pn.name)) = ANY (ARRAY['ballaststoffe'])
      ORDER BY pn.amount_micrograms DESC LIMIT 1
    ) END DESC NULLS LAST,
