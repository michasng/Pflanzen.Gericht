...
BEGIN
  ...
  WITH filtered_products AS (
    ...
    WHERE
      ...
      AND (p_organic IS NULL OR p_organic = false OR p.is_organic = true)
      AND (
      ...
      AND (p_exclude_allergens IS NULL OR array_length(p_exclude_allergens, 1) IS NULL OR NOT (p.allergens && p_exclude_allergens))
  ),
  ingredient_stats AS (
    SELECT
      pi.product_id,
