...
- When importing a primitive purely for its default look, alias the import to its conceptual name for a terser template tag, e.g. `import Button from '@/components/primitives/ButtonComponent.vue'` used as `<Button>`
...
- Do not unit test declarations, when the test only duplicates their values
